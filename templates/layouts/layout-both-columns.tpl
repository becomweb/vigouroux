  ...
  <body id="{$page.page_name}" class="{$page.body_classes|classnames}{if $page.page_name != 'index'} is-not-index{/if}{if $configuration.is_catalog} catalog-mode-on{/if}">
    ...
    <main id="page" urls_pages_cart="{$urls.pages.cart}" static_token="{$static_token}">
      ...
      <section id="wrapper">
        ...
        {/if}
        {* Ajout d'une condition pour ne pas injecter la classe "container" sur la page "Notre histoire" (c-a-d "About us") pour pouvoir y mettre des bandeaux full-width *}
        <div class="{if $page.page_name == 'index'}homepage_column{elseif $page.page_name == 'cms'}{if isset($cms.id) && $cms.id != '4'}container{/if}{else}container{/if} clearfix">
